    ...
    with tf.variable_scope(scope, 'InceptionResnetV2', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d],
                ...
                # 149 x 149 x 32
                net = slim.conv2d(inputs, 32, 3, stride=1, padding='VALID',
                                  scope='Conv2d_1a_3x3')
...
if __name__ == '__main__':
    inputs = tf.placeholder(dtype=tf.float32, shape=[None, 160, 160, 3], name='input_place')
    nets = inference(inputs,0.4,phase_train=True)
    count = 0
